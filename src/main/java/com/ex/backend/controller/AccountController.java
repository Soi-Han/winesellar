...
//			String token = jwtService.getToken("id", id);//토큰만들기.그냥 쿠키에 id값을 토큰이라 치고 넣음.
	...
	public ResponseEntity signup(@RequestBody Optional<Member> member, HttpServletResponse res) {

		Optional<Member> isExist = memberRepository.findByEmail(member.get().getEmail());
		System.out.println(isExist.toString());
		if(isExist.isEmpty()){//없다면
			System.out.println("가입시작");
			Member newmember=new Member();
			newmember.setEmail(member.get().getEmail());
			newmember.setPassword(member.get().getPassword());
			newmember.setName(member.get().getName());
			memberRepository.save(newmember);//저장
			System.out.println("가입");
			return  new ResponseEntity<>(newmember, HttpStatus.OK);
		}
		
		else return new ResponseEntity<>("중복됨", HttpStatus.BAD_REQUEST);
	}
}
